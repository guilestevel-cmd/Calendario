...
function mostrarAvisoDatosIncompletos(cantidad) {
  if (document.getElementById('franja-datos-incompletos')) return;
  const raiz = document.getElementById('app');
  const franja = document.createElement('div');
  franja.id = 'franja-datos-incompletos';
  franja.className = 'franja-aviso';
  franja.textContent = `Se ignoraron ${cantidad} registro(s) con datos incompletos en el Google Sheet (revisa las hojas "Unidades" o "Actividades" por filas con fechas vacías o mal escritas).`;
  raiz.prepend(franja);
}
...
async function iniciar() {
  ...
  try {
    ...
    const descartadas = (Array.isArray(unidades) ? unidades.length : 0) - estado.unidades.length
      + (Array.isArray(actividades) ? actividades.length : 0) - estado.actividades.length;
    if (descartadas > 0) mostrarAvisoDatosIncompletos(descartadas);
